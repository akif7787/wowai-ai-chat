...
import { ChatService } from './services/ai/chat.ts';
import type { ChatRequestPayload } from './services/ai/chat.ts';
...
// Clean up stale rate limit entries every 5 minutes without holding event loop open
...
/**
 * Creates and configures the core Express application with all API routes.
 * Compatible with both standalone Node.js server (server.ts) and Vercel Serverless (api/index.ts).
 */
export function createExpressApp() {
  ...
  // Status endpoint (supports both /api/status and /status for Vercel rewrites)
  ...
  // Model inspection endpoint (supports both /api/models and /models for Vercel rewrites)
  ...
  // Chat endpoint (supports both /api/chat and /chat, SSE streaming and JSON response)
  app.post(['/api/chat', '/chat'], async (req: Request, res: Response) => {
    ...
    if (wantsStream) {
      // Set SSE headers optimized for both standalone servers and Vercel edge/proxy layers
      res.setHeader('Content-Type', 'text/event-stream; charset=utf-8');
      res.setHeader('Cache-Control', 'no-cache, no-transform');
      res.setHeader('Connection', 'keep-alive');
      res.setHeader('X-Accel-Buffering', 'no'); // Disable Nginx/Vercel proxy buffering for real-time SSE
      res.flushHeaders?.();
